...
const code = await satria.requestPairingCode(phoneNumber.trim())
...
satria.readMessages([mek.key])
require("./message/case.js")(satria, m, chatUpdate, store)
} catch (err) {
console.log(err)
}
})
...
let nomer = await satria.sendTextWithMentions(kopel.from, `*${global.botname}* de wa bideo o uketoru koto ga dekimasen. Gomennasai @${kopel.from.split('@')[0]} anata wa bāsaremasu. Ōnā ni renraku shite, burokku o akemasu!`)
satria.sendContact(kopel.from, "6281316701742", nomer)
await sleep(10000)
await satria.updateBlockStatus(kopel.from, "block")
}
}
}
})
...
satria.ev.on("groups.update", async (anu) => {
let { getBuffer } = require('./lib/myfunc')
try {
let ppgroup = await satria.profilePictureUrl(anu.id, 'image').catch(_ =>'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png?q=60')
const gcppg = await getBuffer(ppgroup)
if (anu.announce == true) {
satria.fuck(anu.id, `Group has been closed by admin. Now only admins can send messages!`, "「 Group Settings Change 」", gcppg);
} else if (anu.announce == false) {
satria.fuck(anu.id, `Group has been opened by admin. Now all member can send messages!`, "「 Group Settings Change 」", gcppg);
} else if (anu.restrict == true) {
satria.fuck(anu.id, `Group info has been restricted. Now only admins can edit group info!`, "「 Group Settings Change 」", gcppg);
} else if (anu.restrict == false) {
satria.fuck(anu.id, `Group info has been opened. Now participants can edit group info!`, "「 Group Settings Change 」", gcppg);
} else {console.log(anu)}
} catch (err) {
console.log(err);
}
});  
...
if (satria.user && satria.user.id) satria.user.jid = satria.decodeJid(satria.user.id)
...
if (id in satria.contacts && satria.contacts[id][isGroup ? 'subject' : 'name'] && id in satria.chats) return
let metadata = {}
if (isGroup) metadata = await satria.groupMetadata(id)
let chat = { ...(satria.contacts[id] || {}), id, ...(isGroup ? { subject: metadata.subject, desc: metadata.desc } : { name }) }
...
const chat = satria.decodeJid(m.key.remoteJid || m.message?.senderKeyDistributionMessage?.groupId || '')
...
const groups = await satria.groupFetchAllParticipating().catch(_ => null) || {}
for (const group in groups) satria.chats[group] = { ...(satria.chats[group] || {}), id: group, subject: groups[group].subject, isChats: true, metadata: groups[group] }
return satria.chats
}
...
try {
if (!message) continue
if (message.messageStubType && message.messageStubType != WAMessageStubType.CIPHERTEXT) satria.processMessageStubType(message).catch(console.error)
const _mtype = Object.keys(message.message || {})
const mtype = (!['senderKeyDistributionMessage', 'messageContextInfo'].includes(_mtype[0]) && _mtype[0]) ||
(_mtype.length >= 3 && _mtype[1] !== 'messageContextInfo' && _mtype[1]) ||
_mtype[_mtype.length - 1]
const chat = satria.decodeJid(message.key.remoteJid || message.message?.senderKeyDistributionMessage?.groupId || '')
...
const qM = {
key: { remoteJid, fromMe: areJidsSameUser(satria.user.jid, remoteJid),id: context.stanzaId, participant, },
message: JSON.parse(JSON.stringify(quoted)), ...(isGroup ? { participant } : {})
}
let qChats = satria.chats[participant]
if (!qChats) qChats = satria.chats[participant] = { id: participant, isChats: !isGroup }
...
chats = satria.chats[chat] = { id: chat, isChats: true, ...(satria.chats[chat] || {}) }
}
let metadata, sender
if (isGroup) {
if (!chats.subject || !chats.metadata) {
metadata = await satria.groupMetadata(chat).catch(_ => ({})) || {}
if (!chats.subject) chats.subject = metadata.subject || ''
if (!chats.metadata) chats.metadata = metadata
}
sender = satria.decodeJid(message.key?.fromMe && satria.user.id || message.participant || message.key?.participant || chat || '')
if (sender !== chat) {
...
const fromMe = message.key.fromMe || areJidsSameUser(sender || chat, satria.user.id)
...
satria.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted })
}
...
return satria.sendMessage(jid, { text: teks,
contextInfo: { 
...
satria.sendTextWithMentions = async (jid, text, quoted, options = {}) => satria.sendMessage(jid, { text: text, contextInfo: { mentionedJid: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net') }, ...options }, { quoted })
...
return satria.sendMessage(jid, { image: await getBuffer(url), caption: caption, ...options },{quoted});
}
...
return await satria.sendMessage(jid, {
contacts: {
...
await satria.sendMessage(jid, { sticker: { url: buffer }, ...options }, { quoted })
return buffer
}
...
return buffer}
 ...
 satria.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
...
await satria.sendMessage(jid, { [type]: { url: pathFile }, mimetype, fileName, ...options }, { quoted, ...options })
...
satria.copyNForward = async (jid, message, forceForward = false, options = {}) => {
let vtype
...
...content[ctype].contextInfo,
...options.contextInfo}} : {})} : {})
await satria.relayMessage(jid, waMessage.message, { messageId:  waMessage.key.id })
return waMessage}
